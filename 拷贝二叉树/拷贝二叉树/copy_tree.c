...
//拷贝二叉树
Binarynode* copy_tree(Binarynode* root)
{
	if (root = NULL)
	{
		return;
	...
	//拷贝左子树
	Binarynode* lchild = copy_tree(root->lchild);
	//拷贝右子树
	Binarynode* rchild = copy_tree(root->rchild);
...
//释放二叉树
void free_tree(Binarynode* root)
{
	if (root == NULL)
	{
		return;
...
int main()
{
	...
	//复制二叉树
	Binarynode* root = copy_tree(&node1);
	//先序遍历
	recursion(root);
	free_tree(root);
	system("pause");
